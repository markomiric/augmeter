...
export function computeStatusColorWithAccessibility(
  ...
  // Caution - only show if we have real data, use prominent foreground for readability
  if (percentage >= adjustedThresholds.caution && hasRealData) {
    return "statusBarItem.prominentForeground";
  }

  // Normal usage with real data
  if (hasRealData) {
    return "statusBarItem.prominentForeground";
  }

  // No real data - use default theme color
  return undefined;
}
...
function computeStandardColors(
  ...
  // Caution - only show if we have real data
