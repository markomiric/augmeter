...
export class AugmentApiClient {
  ...
  async sendUsageEvents(
    events: Array<{ event: string; ts: string; properties?: Record<string, any> }>
  ): Promise<AugmentApiResponse> {
    // Provisional endpoint; server may add this in future. We still call it safely.
    try {
      return await this.makeRequest("/usage/events", {
        method: "POST",
