...
.mermaid-fullscreen.is-active {
  opacity: 1;
  visibility: visible;
}

.mermaid-fullscreen__header {
  display: flex;
...
.mermaid-fullscreen__body .mermaid {
  transition: transform 0.2s ease;
...
```html
<!-- Fullscreen overlay (add once per page, before closing </body>) -->
<div class="mermaid-fullscreen" id="mermaidFullscreen">
  <div class="mermaid-fullscreen__header">
    <div class="zoom-controls">
      <button onclick="zoomFullscreen(1.2)" title="Zoom in">+</button>
      <button onclick="zoomFullscreen(0.8)" title="Zoom out">&minus;</button>
      <button onclick="resetFullscreenZoom()" title="Reset zoom">&#8634;</button>
    </div>
    <button class="btn-close-fullscreen" onclick="closeFullscreen()" title="Close">&times;</button>
  </div>
  <div class="mermaid-fullscreen__body" id="mermaidFullscreenBody"></div>
</div>

<!-- Each mermaid diagram -->
<div class="mermaid-wrap">
  <div class="zoom-controls">
    <button onclick="zoomDiagram(this, 1.2)" title="Zoom in">+</button>
    <button onclick="zoomDiagram(this, 0.8)" title="Zoom out">&minus;</button>
    <button onclick="resetZoom(this)" title="Reset zoom">&#8634;</button>
    <button onclick="openFullscreen(this)" title="Expand">&#x26F6;</button>
  </div>
  <pre class="mermaid">
    graph TD
      A --> B
...
function openFullscreen(btn) {
  ...
  if (!svg) return;
  var overlay = document.getElementById("mermaidFullscreen");
  var body = document.getElementById("mermaidFullscreenBody");
  body.innerHTML = "";
...
function zoomFullscreen(factor) {
  ...
  if (!svg) return;
  fullscreenZoom = Math.min(Math.max(fullscreenZoom * factor, 0.3), 5);
  svg.style.transform = "scale(" + fullscreenZoom + ")";
}
...
Scroll-to-zoom requires Ctrl/Cmd+scroll to avoid hijacking normal page scroll. Click-and-drag panning activates only when zoomed in (zoom > 1). Cursor changes to `grab`/`grabbing` to signal the behavior. The zoom range is capped at 0.3x-5x. The fullscreen expand button clones the rendered SVG into a fixed overlay for comfortable viewing of complex diagrams. Escape key closes the overlay.
