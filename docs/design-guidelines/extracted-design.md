...
Extracted from the current connected-dashboard documentation capture rendered on August 11, 2026. The capture uses illustrative data generated through the production dashboard renderer; it contains no local history, provider account data, or credentials.
...
- Typography: VS Code system sans-serif; headings use 600 weight, primary metrics use 700 weight, and metadata uses smaller muted text with uppercase source labels.
...
- At 520px and below, use one column, 12px page padding, and 18px primary metrics.
...
- Keep the disconnected notice's `role="status"` and polite live region behavior.
